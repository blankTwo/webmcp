...
const listedBackground = manager.list("workspace-a", false).find((process) => process.sessionId === background.sessionId);
assert.ok(listedBackground);
assert.equal(listedBackground.running, true);
assert.equal(typeof listedBackground.pid, "number");
assert.match(listedBackground.command, /finished/);
assert.equal(manager.get("workspace-a", background.sessionId).cwd, process.cwd());
...
let firstPeek = manager.peek("workspace-a", background.sessionId);
for (let attempt = 0; attempt < 40 && !/finished/.test(firstPeek.output); attempt += 1) {
  await new Promise((resolve) => setTimeout(resolve, 50));
  firstPeek = manager.peek("workspace-a", background.sessionId);
}
const secondPeek = manager.peek("workspace-a", background.sessionId);
assert.match(firstPeek.output, /finished/);
assert.equal(secondPeek.output, firstPeek.output);

let completed = await manager.write({
  workspaceId: "workspace-a",
  sessionId: background.sessionId,
  yieldTimeMs: 2_000,
});
...
const completedInfo = manager.get("workspace-a", background.sessionId);
assert.equal(completedInfo.running, false);
assert.equal(completedInfo.exitCode, 0);
assert.equal(manager.list("workspace-a", false).some((process) => process.sessionId === background.sessionId), false);
assert.equal(manager.list("workspace-a", true).some((process) => process.sessionId === background.sessionId), true);
...
const terminatedInfo = await manager.terminate("workspace-a", terminable.sessionId, 500);
assert.equal(terminatedInfo.running, false);
...
if (buffered.sessionId) await manager.terminate("workspace-a", buffered.sessionId, 500);
