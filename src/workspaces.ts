...
export class WorkspaceRegistry {
  ...
  async memoryKey(workspace: Workspace): Promise<string> {
    return JSON.stringify([workspace.mode, await canonicalPath(workspace.root)]);
  }
  ...
  async searchCheckpoints(
    ...
    return this.store.searchCheckpoints(await this.memoryKey(workspace), query, limit);
  }

  async getMemorySnapshotByRoot(
    root: string,
    ...
    if (!this.store) return { checkpoints: [] };
    const workspaceKey = await this.memoryKeyForRoot(root, mode);
    return {
      resumeState: this.store.getResumeState(workspaceKey),
      checkpoints: this.store.searchCheckpoints(workspaceKey, "", limit),
    };
  }

  async clearResumeStateByRoot(
    root: string,
    mode: WorkspaceMode = "checkout",
  ): Promise<boolean> {
    if (!this.store) return false;
    return this.store.deleteResumeState(await this.memoryKeyForRoot(root, mode));
  }

  private async memoryKeyForRoot(root: string, mode: WorkspaceMode): Promise<string> {
    const allowedRoot = mode === "worktree"
      ? assertAllowedPath(root, [this.config.worktreeRoot])
      : assertAllowedPath(root, this.config.allowedRoots);
    return JSON.stringify([mode, await canonicalPath(allowedRoot)]);
  }
