...
assert.deepEqual(
  getToolInputDisplay("bash", { command: "npm run typecheck" }),
  {
...
assert.deepEqual(
  getToolInputDisplay("read", { path: "src/server.ts" }),
  {
...
assert.deepEqual(
  getToolInputDisplay("bash", { command: "npm test" }, "cancelled"),
  {
    icon: toolIcons.terminalSquare,
...
assert.deepEqual(
  getToolHeaderSummary(getToolInputCard("edit", { path: "src/server.ts" })),
  { kind: "text", text: "Running" },
);
assert.deepEqual(
  getToolDisplay({ tool: "bash", status: "error", summary: { command: "npm test" } }),
  {
    icon: toolIcons.terminalSquare,
    title: "Bash failed",
    label: "npm test",
...
assert.deepEqual(
  getToolHeaderSummary({ tool: "read", status: "error", path: "missing.ts" }),
  { kind: "text", text: "Failed" },
);
assert.equal(
  getToolInputCard("grep", { pattern: "registerAppTool", path: "src" }).summary?.scope,
  "src",
);
