...
async function boot(): Promise<void> {
  ...
  app.ontoolinput = (input) => {
    updateRunningTool(input.arguments, false);
  };
  ...
  app.ontoolresult = (result) => {
    ...
    const structured = metaCard
      ...
      : structuredContent;
    const tool = toolNameFromMeta(result) ?? declaredTool ?? card?.tool;
    ...
    const inputCard = getToolInputCard(tool, toolArguments);
    const errorContent = result.isError ? toolContentFromResult(result) : [];
    const nextCard: ToolResultCard = {
      ...inputCard,
      ...(errorContent.length > 0 ? { payload: { content: errorContent } } : {}),
      ...(isToolResultCard(structured) ? structured : {}),
      tool,
      status: result.isError ? "error" : undefined,
    };
    card = nextCard;
    expanded = result.isError && isExpandableCard(nextCard)
      ? true
  ...
  app.ontoolcancelled = (cancelled) => {
    const tool = declaredTool ?? card?.tool;
    if (!tool) {
      resetToolCard();
      render();
      return;
    }
    card = getToolInputCard(tool, toolArguments, "cancelled");
    if (cancelled.reason) {
      card.summary = { ...card.summary, cancelReason: cancelled.reason };
    }
    expanded = false;
    errorMessage = null;
    render();
...
function updateRunningTool(
  ...
  toolArguments = partial
    ? { ...toolArguments, ...(nextArguments ?? {}) }
    : { ...(nextArguments ?? {}) };
  card = getToolInputCard(tool, toolArguments);
  expanded = false;
  reviewFilesExpanded = false;
  openWorkspaceInstructionKey = null;
  showAvailableWorkspaceInstructions = false;
...
function toolContentFromResult(result: CallToolResult): Array<{
  type: "text" | "image";
  text?: string;
  data?: string;
  mimeType?: string;
