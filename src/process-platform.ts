...
export function terminateProcessTree(
  ...
  if (runtime.platform === "win32" && child.pid) {
    // Windows shell commands can leave descendants orphaned if only cmd.exe is
    // signalled. Always terminate the whole tree atomically on this platform.
    if (runtime.killWindowsTree(child.pid, true)) return;
