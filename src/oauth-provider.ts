...
function formHtml(params: {
  ...
  const scopeText = params.scopes.length > 0 ? params.scopes.join(" ") : "gptmcp";
  const resourceText = params.resource?.href ?? "GPTMCP MCP endpoint";
...
${hiddenFields}
        ...
        <button type="submit">Authorize GPTMCP</button>
      </form>
    </main>
  </body>
</html>`;
