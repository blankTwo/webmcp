...
try {
  ...
  assert.equal(updated.workerPid, 12345);
  ...
  store.update(created.id, { status: "stopped" });
  assert.equal(
    store.updateUnlessStopped(created.id, { status: "idle", latestResponse: "should-not-win" }).status,
    "stopped",
  );
  assert.equal(store.get(created.id)?.latestResponse, undefined);
  assert.deepEqual(
