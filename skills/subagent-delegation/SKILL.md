...
Use DevSpace's native MCP tools for normal delegation:

- `run_agent` starts a configured profile or raw built-in provider. Passing an
  existing agent id sends a follow-up to that persistent provider session.
- `get_agent` reads the latest status, response, or error for one agent.
- `list_agents` lists sessions scoped to the current workspace.
- `cancel_agent` terminates a running agent worker and its child process tree.

Do not invoke `devspace agents ...` through shell for normal MCP delegation.
The CLI remains available for terminal users and integration debugging. Do not
run provider CLIs such as `codex`, `claude`, `opencode`, `pi`, `cursor-agent`,
or `copilot` directly unless you are explicitly debugging DevSpace agent
integration.

## Choosing a profile

Choose profiles from the compact subagent profile catalog returned by
`open_workspace`. Pass the profile name as `run_agent.target`. If no profile
fits and delegation is still appropriate, use a built-in provider name from
`open_workspace`.

Profiles may declare a model and optional thinking level. Override those only
through the optional `run_agent.model` or `run_agent.thinking` fields.
