...
GPTMCP can save a file attached or generated by an MCP host, such as ChatGPT,
directly into an open workspace. Enable the tool with `GPTMCP_ARTIFACTS=1`.
...
3. Use the returned path with the ordinary GPTMCP filesystem tools.
...
GPTMCP creates missing parent directories and refuses to overwrite an existing
file. After the download, normal tools can inspect, move, rename, replace, or
delete it.

## Safety and limits

The `file` input must be the native value supplied by the MCP host. GPTMCP does
...
Downloads are streamed under `GPTMCP_ARTIFACT_MAX_FILE_BYTES` and published as
