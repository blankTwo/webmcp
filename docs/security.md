...
WebMCP exposes local coding capabilities over MCP. Treat it as remote access
...
WebMCP only opens workspaces under configured roots.
...
`webmcp init` generates an Owner password and stores it in:

```text
~/.webmcp/auth.json
```

When an MCP client connects, WebMCP shows an approval page. Enter the Owner
...
WebMCP needs `WebMCP_PUBLIC_BASE_URL` so MCP clients can discover OAuth
...
Do not include `/mcp` in `WebMCP_PUBLIC_BASE_URL`.

By default, WebMCP derives allowed Host headers from the local host and public
URL. Use `WebMCP_ALLOWED_HOSTS=*` only for intentional local debugging.

## Tunnels

WebMCP does not manage tunnels. Your tunnel or reverse proxy should point to:
...
protection in front of public tunnels. WebMCP OAuth still protects the MCP
...
Filesystem path containment applies to WebMCP file tools. Shell commands run
...
WebMCP accepts only the documented native-file object and trusted OpenAI
...
published without overwrite as owner-only files. WebMCP does not extract or
...
`GET /statusz` exposes local operational details used by `webmcp status` and
`webmcp restart`, including the running process command and effective
non-secret configuration. It requires the Owner password in the
`x-webmcp-owner-token` header and returns `Cache-Control: no-store`. The Owner
password itself is never returned by either endpoint or written to request logs.

## Logs

By default, WebMCP logs requests and tool calls. Shell command previews are
disabled unless `WebMCP_LOG_SHELL_COMMANDS=1`.
