...
Stable profile identifier shown to the model and accepted as `run_agent.target`.
The terminal CLI also accepts the same name with `devspace agents run`.
...
When Subagents are enabled, the model uses native MCP tools:

- `run_agent` to start a profile/provider or continue an existing agent id.
- `get_agent` to read status and the latest final response.
- `list_agents` to inspect sessions in the current workspace.
- `cancel_agent` to terminate a running worker process tree.
...
`list_agents` lists existing subagent sessions for the current workspace; it
does not list profile definitions. The `devspace agents` CLI remains available
for terminal users and integration debugging.
...
- Exposing provider-specific launch mechanics through the MCP schema; native
  agent tools stay provider-neutral and reuse the same adapter registry as the
  terminal CLI.
